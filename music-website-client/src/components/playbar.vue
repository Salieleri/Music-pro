  ...
  methods: {
    ...
    prev () {
      if (this.listIndex !== -1 && this.listOfSongs.length > 1) {
        if (this.listIndex > 0) {
          ...
          this.toPlay(this.listOfSongs[this.listIndex].songPos)
        } else {
    ...
    next () {
      if (this.listIndex !== -1 && this.listOfSongs.length > 1) {
        if (this.listIndex < this.listOfSongs.length - 1) {
          ...
          this.toPlay(this.listOfSongs[0].songPos)
        }
      }
    },
    // 选中播放
    toPlay (url) {
      if (url && url !== this.url) {
        ...
        this.$store.commit('setArtist', this.listOfSongs[this.listIndex].songSinger)
        this.$store.commit('setLyric', this.listOfSongs[this.listIndex].songLyric)
        if (this.loginin) {
          let mess = {
            'userid': this.userid,
            'id': this.listOfSongs[this.listIndex].songId
          }
    ...
    goPlayerPage () {
      this.$router.push({path: `/songcomment/${this.id}`})
    },
    collection () {
      if (this.loginin) {
        let mess = {
...
<style scoped>
@import '../assets/scss/playbar.scss';
</style>
